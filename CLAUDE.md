...
**Ao criar um novo domínio, replique essa fatia vertical completa** (entity → repository → mapper → request/DTO → service → controller e `<Dominio>Docs` → migration → testes de service e de controller).
...
* **Documentação do Swagger fica na interface `<Dominio>Docs`**, no mesmo pacote do controller, que a implementa. A interface concentra o `@Tag`, os mapeamentos (`@GetMapping`, `@PostMapping`...), `@RequestBody @Valid`, `@PathVariable`, `@PageableDefault` e a documentação de cada rota: `@Operation` com `summary` e uma `description` que termina com a linha **Perfis com acesso**, e `@ApiResponses` com todos os códigos que a rota devolve (os de erro apontam para `ErrorResponseDTO`). O controller mantém só `@RestController`, `@RequestMapping` e métodos `@Override` sem anotação de parâmetro. Os métodos se chamam pela ação (`listarMedicos`, `salvarMedico`, `atualizarMedico`, `deletarMedico`, `cancelarAgendamento`), não pelo verbo HTTP. Listagens paginadas levam `@ParametrosPaginacao` (pacote `controller`) e escondem o `Pageable` com `@Parameter(hidden = true)`, porque a documentação padrão do springdoc descreve a página começando em 0. O login usa `@SecurityRequirements` vazio para aparecer como rota pública.
* **Todo campo de request e DTO leva `@Schema` com `description` e `example`.** Campos de data acrescentam `type = "string"` e `format` com o padrão do `@JsonFormat` (`dd/MM/yyyy` ou `dd/MM/yyyy - HH:mm:ss`). Sem isso o Swagger sugere o formato ISO e o "Try it out" envia um valor que o Jackson recusa com 400. Ao mudar uma regra de negócio, de autorização ou um código de resposta, atualize a descrição da rota na interface `Docs` na mesma mudança.
* **Cada service injeta só o próprio repositório.** Quando precisa de outro domínio, vai pelo service dele (`medicoService.getMedicoById(...)`, `pacienteService.getPacienteById(...)`, `pacienteService.getPacienteByLogin(...)`), nunca pelo repositório alheio. É o que mantém a fatia vertical fechada e o log da busca no domínio dono do dado.
...
* **Agenda do paciente:** `validarDisponibilidadeDoPaciente(...)` recusa marcar (`salvar`) ou remarcar (`atualizar`) uma consulta se o paciente já tiver outra no **mesmo `dataHoraConsulta`** — `existsByPacienteIdAndDataHoraConsulta` e, na remarcação, `existsByPacienteIdAndIdNotAndDataHoraConsulta`, que desconsidera a própria consulta pelo `IdNot`, exatamente como a agenda do médico. Conflito lança `HorarioConsultaIndisponivelException`, que o `GlobalExceptionHandler` traduz em `409 Horário de Consulta Indisponível!`. As duas validações rodam sempre em par — paciente primeiro, médico depois —, então `POST` e `PATCH` aplicam a mesma regra. Por isso `dataHoraConsulta` é `@NotNull` no `SalvarAgendamentoRequest` (exceção à regra de que só `Salvar…Request` de cadastro tem campo obrigatório). Do lado do médico **não existe** checagem de horário exato: o intervalo mínimo de 1 hora é aberto nas pontas e já contém o próprio horário, e a constraint `uk_agendamento_medico_horario` é a última barreira — acrescentar um `existsByMedicoIdAndDataHoraConsulta` só duplicaria a consulta e esconderia a mensagem do intervalo mínimo.
...
* `GET` (ver consultas) → `MEDICO`, `RECEPCIONISTA`, `ENFERMEIRO` ou `PACIENTE`. É o **único** endpoint que a role `PACIENTE` acessa. **Quando quem chama tem a role `PACIENTE`, o `GET` devolve só as consultas do próprio paciente** — `AgendamentoService.getAgendamentos(...)` recebe o `Authentication`, e se a role for `PACIENTE` resolve o `paciente` por `pacienteService.getPacienteByLogin(authentication.getName())` e filtra por `agendamentoRepository.findByPacienteId(...)`; login de paciente sem cadastro correspondente recebe `Page.empty(...)`, ou seja, lista vazia. Os demais papéis veem todas.
