...
class AuthControllerTest {
    ...
    void setUp() throws Exception {
        ...
        loginRequest = new String(Files.readAllBytes(
                Paths.get("src/test/resources/auth/loginRequest.json")
        ));

        loginSemLoginRequest = new String(Files.readAllBytes(
                Paths.get("src/test/resources/auth/loginSemLoginRequest.json")
        ));

        loginSemSenhaRequest = new String(Files.readAllBytes(
                Paths.get("src/test/resources/auth/loginSemSenhaRequest.json")
        ));

        loginUsuarioInexistenteRequest = new String(Files.readAllBytes(
                Paths.get("src/test/resources/auth/loginUsuarioInexistenteRequest.json")
        ));

        loginSenhaIncorretaRequest = new String(Files.readAllBytes(
                Paths.get("src/test/resources/auth/loginSenhaIncorretaRequest.json")
        ));
    }
        ...
        when(authService.login(any(LoginRequest.class)))
                .thenThrow(
                        new UsuarioNaoEncontradoException(
                                "Usuário com login 'inexistente' não encontrado!"
                        )
                );
